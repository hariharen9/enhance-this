...
def mock_ollama_client():
    # Patch the name as bound inside cli (from .ollama_client import OllamaClient)
    # so no real network I/O ever happens during CLI tests.
    with patch('enhance_this.cli.OllamaClient') as MockOllamaClient:
...
def mock_config_file(tmp_path):
    # Keep tests hermetic: route config + template reads/writes to a temp dir.
...
def test_cli_version(runner):
    ...
    assert "enhance, version" in result.output
...
def test_cli_enhance_output_file(runner, mock_ollama_client, tmp_path):
    ...
    assert result.exit_code == 0
    # click.File prints the full resolved path via the file's .name.
